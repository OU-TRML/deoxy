...
/// Since testing can occur outside of the testing environment (a la documentation tests), this
/// uses `feature = "stub"` instead of the `test` config flag.
...
/// Specialized `Result` for `Pin` methods.
...
impl Pin {
    ...
    #[cfg(not(feature = "stub"))]
    #[cfg_attr(not(feature = "stub"), doc = "Constructs a new stub.")]
    pub fn new(number: u16) -> Self {
        Self {
    ...
    /// # Panics
    /// This method will panic if `width` >= `total` (that is, the wave is as wide as the period or
    /// wider), as this badly malformed of input is likely not somehing that can be recovered from.
    ///
    /// In general, this method panics if the input is malformed, returning `Result::Err` only in
    /// the case of errors from the GPIO underpinnings.
    pub fn do_wave(&mut self, width: Duration, total: Duration) -> PinResult {
        assert!(
            total > width,
            "Wave pulse of {:?} exceeds period of {:?}.",
            width,
            total
        );
        self.set_high()?;
